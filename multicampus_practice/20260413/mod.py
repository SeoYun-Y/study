class Bank:
    ...
    total_cost = 0  # 유저가 입금/ 출금시 total_cost를 변화 ( class 생성시 _cost를 total_cost에 누적합)
...
# 일의 종류 ->
    ...
    # DB(데이터들을 모아주는 공간)를 이용하여 데이터를 로드
...
# ( 같은 class를 생성했을 때 일의 종류와 물건의 종류는 서로 공유가 가능하게 하기위해 )
...
class User(Bank):
    ...
    def __init__(self,_name,_birth,_cost):
        ...
        # super().__init__() -> 생성되는 변수의 개수는 ? -> 4개(이름,생년월일,잔액,거래내역)
        # 구매한 아이템의 목록 -> 빈 리스트로 생성
        self.items = []

    # 일을한다... 함수(매개변수: 일의 종류, 시간)
    def work(self, _type, _time):
        # _type이 work_types에 데이터가 존재하는가?
        ...
        if _type in User.work_type:
            ...
            # print(amount)       # 일당을 출력
            # 일당이 계산이 되었으니 일당을 통장(Bank -> 부모클래스 -> suepr())에 입금(add_cost(금액))
    ...
    def buy_item(self,_item):
        ...
        try:
            # 시도할 코드
            # 물건의 가격을 로드
            # 물건의 가격은 User 클래스 안에 item_list에 dict 형태로 존재 -> key을 이용해서 가격을 추출 dict[key]
            # key로 사용될 데이터는 ? -> _item
            # print(User.item_list)
            # print(User.item_list[_item])    # 물건의 가격을 추출
            # 가격을 알았으니 출금 ( Bank안에 sub_cost())
            ...
            # as : 별칭을 짓겠다
            # e : 별칭 명
            print(type(e)._name) # 어떤 에러가 발생했는지 print() 출력
            # try 영역에서 실행되는 코드가 예외가 발생했을때만 실행이 되는 영역
            print('물건의 정보가 없습니다.')
    # 오버라이드 : 부모클래스의 기능의 이름을 덮어씌워서 사용
    # (부모클래스에서는 1이라는 기능을 하고 자식 클래스에서는 2라는 기능을 한다) ->
    # 부모 클래스의 함수의 기능은 그대로 유지한다
    # 이름, 생년월일, 잔액, 구매한 물건의 목록을 출력
    @override   # 문법적으로 오버라이드 함수이다 -> 가독성이 좋게 표시
    def user_info(self):
        print(f'이름은 : {self.name}, 생년월일 : {self.birth}, 잔액 : {self.cost}, 구매한 물건의 목록 : {self.items}')
    # User 클래스에서는 이름 생년월일 잔액 물건의 구매목록을 출력
